...
def eig_blocks(H0block, n_dim0):
    ...
    for H_ in H0block: 
        if len(H_) == 0:
            ...
            i_uv = np.argwhere(aux_val-eigval[0] <= 50.).flatten() #45 se nao for pra conductance
    ...
    for i, q in enumerate(range(0,2*Qmax0+1, 2)):
        for d in range(0, 3+N-i):
            ...
            if len(index) == 0: #Fixes diagonal ordering for these subspaces. 
                ...
                H_block.append(fills_NonDiagonal(N, 2.*E_father/2., Space[q+d,d], fn0, d))    
            n += 1
...
def fills_NonDiagonal(N, E, QS, fn, yS):
    """
    ...
    coef = np.sqrt(2)  #coeficient that accompanies non diagonal term.
    if N<3:
        coef = [-1./np.sqrt(2), 1.][N-1]
...
def iterative_fn(N, dE, fN):
    ...
    fn2 = fN
    fn1 = np.multiply(dE, fN)/np.sqrt(2)

    if N<=2 and N>0:
        fn1 = coef[N-1]*np.multiply(dE, fN)
    ...
    for n in range(N-3, -2, -1):
        if n<1:
            coef2 = [1/np.sqrt(2), -1/np.sqrt(2)]
            fn = 2**(2-N)*np.multiply(dE, fn1)*coef[n+1] + fn2*coef2[n+1]

        else:
            fn = 2**(n+2-N)*np.multiply(dE, fn1)/np.sqrt(2) - fn2/2

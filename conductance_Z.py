...
from NRG_Z import iterative_diagonalization, u_v_vg_w, iteration_0, iteration_N, matrix_elements
...
def conductance_ll_(N, B, Ef_, Ep_, f0):
    Gll = 0
    i = 0
    for Ep in Ep_:
        for Ef in Ef_:
...
def print_energies(E0, N, folder):
    ...
    file = open("resultados/resultados" + str(folder) +"/energies/" + str(N) + "_z.txt", "w+")
...
def dynamical_proprieties(Nmax, folder):
    ...
    file = open("resultados/resultados"+ str(folder)+"/L0_z.txt", "w")
    for i, G in enumerate(L0N):
        N = i+1
        np.savetxt(file, np.c_[1/(B_*2**(N-2)), G, B_])
    file.close()

    file = open("resultados/resultados"+ str(folder)+"/L1_z.txt", "w")
    for i, S in enumerate(L1N):
        N = i+1
        np.savetxt(file, np.c_[1/(B_*2**(N-2)), S, B_])
    file.close()

    file = open("resultados/resultados"+ str(folder)+"/L2_z.txt", "w")
    for i, K in enumerate(L2N):
        N = i+1
        np.savetxt(file, np.c_[1/(B_*2**(N-2)), K, B_])
    file.close()

    file = open("resultados/resultados"+ str(folder)+"/L01_z.txt", "w")
